...
export default function CommunityError({
  ...
  useEffect(() => {
    console.error("Community page error:", error.message, error.digest, error.stack);
  }, [error]);
  ...
  return (
    <div className="min-h-screen bg-nu-paper flex items-center justify-center px-8">
      <div className="max-w-lg w-full bg-nu-white border-[2px] border-nu-ink/[0.08] p-10 text-center">
        ...
        <h2 className="font-head text-2xl font-extrabold text-nu-ink mb-3">페이지 로드 오류</h2>
        <p className="text-sm text-nu-muted mb-4 leading-relaxed">
          이 페이지를 불러오는 중 문제가 발생했습니다.<br />
          잠시 후 다시 시도해 주세요.
        </p>
        {error.digest && (
