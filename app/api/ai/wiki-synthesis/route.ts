...
## 핵심 철학
너트(팀)는 하나의 주제로 모인 사람들입니다. 매주 회의와 토론을 통해 **하나의 통합 문서(탭)**를 점진적으로 완성해갑니다.
최종 목표는 논문 수준의 종합 자료입니다.

## 우선순위
1. **회의록/토론 내용이 1순위 자료입니다** — 팀이 실제로 논의하고 결정한 내용이 탭의 근간입니다.
2. **공유 리소스는 보강 자료입니다** — 회의에서 논의된 내용을 뒷받침하는 근거로 활용합니다.
3. **기존 섹션을 강화하세요** — 새 페이지를 만들기보다 기존 섹션(topic)의 내용을 심화·확장하세요.
4. **이미 탭으로 정리된 자료는 재검토하지 않습니다** — 이전 통합의 요약만 컨텍스트로 받고, 새 데이터만 분석합니다.

반드시 아래 JSON 형식으로만 응답하세요:

{
  "weeklyTheme": "이번 주 회의의 핵심 테마 (한 문장)",
  "consolidatedSummary": "이번 주 회의와 자료에서 도출된 핵심 지식 요약 (500자 이내). 이전 맥락과 자연스럽게 이어지게.",
  "wikiPageSuggestions": [
    {
      "title": "탭 페이지 제목 (= 통합 탭의 한 섹션)",
      "content": "전체 마크다운 내용. 회의에서 논의된 핵심 내용을 위키 형식으로 정리. 리소스는 근거로 인용. 최소 300자, 최대 500자.",
      "topicName": "배정할 섹션(토픽) 이름 — 기존 섹션이 있으면 반드시 그 이름 사용",
      "action": "create 또는 update (기존 섹션 강화는 update)",
      "tags": ["태그1", "태그2"],
      "sourceResources": ["참조한 리소스 제목들"],
      "keyInsight": "이 섹션의 핵심 인사이트 (한 문장)"
    }
  ],
  "crossReferences": [
    {
  ...
  "knowledgeGaps": ["회의에서 다루지 못했거나 추가 조사가 필요한 영역"],
  "growthMetrics": {
    "newConceptsIntroduced": 0,
    "conceptsDeepened": 0,
    "connectionsDiscovered": 0
  },
  "nextWeekSuggestions": ["다음 주 회의에서 다룰 만한 주제 3개"],
  "compactionNote": "이전 통합 대비 이번 주에 추가된 핵심 변화 (200자 이내)"
}

규칙:
- 반드시 유효한 JSON만 출력, 한국어로 작성
- **회의록 우선**: 회의 내용이 있으면 이를 중심으로 구성하고, 리소스를 보조 근거로 활용
- **기존 섹션 강화 우선**: 새 섹션(create)보다 기존 섹션 업데이트(update)를 선호
- wikiPageSuggestions는 최대 3개, 각 content는 300-500자
- action이 "update"인 경우 기존 내용에 추가할 새로운 내용만 작성
