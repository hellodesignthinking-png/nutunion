...
export const metadata: Metadata = {
  ...
  description: "nutunion 커뮤니티 프로젝트를 탐색하세요",
};
...
export default async function ProjectsPage() {
  ...
  const canCreate = isAdmin || profile?.can_create_crew;

  // Fetch projects: non-draft for regular users, all for admins
  let query = supabase
    .from("projects")
    .select(
      "*, creator:profiles!projects_created_by_fkey(id, nickname, avatar_url), project_members(count)"
    )
      ...
      <div className="relative bg-nu-ink overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-8 pt-28 pb-16">
          ...
          <p className="text-nu-paper/40 mt-4 max-w-lg text-sm leading-relaxed">
            크루와 멤버들이 함께 만들어가는 프로젝트들입니다. 관심 있는
            프로젝트에 참여하거나 새로운 프로젝트를 시작해보세요.
          </p>
          {canCreate && (
